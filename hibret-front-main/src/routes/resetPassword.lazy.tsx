...
export const Route = createLazyFileRoute('/resetPassword')({
  component: () => {
    ...
    const reset = async (newPassword: string): Promise<ValidationResult> => {
      try {
        const response = await axios.put<ResetResponse>('http://localhost:5000/api/setNewPassword', 
        ...
        if (error.response) {
          ...
          return { isError: true, error: 'Network error' };
        }
      }
    };
