    ...
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 bg-gray-700 bg-opacity-50 overflow-y-auto">
      <div className="bg-white rounded-lg p-8 w-300 overflow-y-auto">
        ...
        <div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mt-4">
              ...
              <Dropdown options={wokflowoptions} onChange={(option) => setSelectedWorkflow(option)} value={selectedWorkflow} placeholder="Select an option" />
            </div>
          </form>
        </div>
        ...
        <div className="mt-8 p-4 bg-gray-100 rounded-lg">
          <h3 className="text-[#00B0AD] text-1xl font-bold">Stages</h3>
          {stages.map((stage, index) => (
            <div key={index} className="mt-4">
              <h3 className="text-[#00B0AD] text-1xl font-bold">Stage {index+1}</h3>
              <div className="mt-4 flex justify-between">
                ...
                <div className="w-1/2 pl-2">
                  ...
                  <Dropdown options={authorizationoptions} onChange={(option) => setSelectedAuth(option)} value={selectedAuth} placeholder="Select an option" />
                </div>
              </div>
            </div>
          ))}
