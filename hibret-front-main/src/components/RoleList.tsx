...
const columns: GridColDef[] = [
  ...
  { field: "permissions", headerName: "permission", width: 230 },
  {
    ...
    renderCell: (params) => {
      ...
      const removeList= existingList.filter((value:any) => allowedPermission.includes(value));
      const addList= allowedPermission.filter((value:any) => !existingList.includes(value));
      ...
      console.log(params.row)
       ...
       function removePermissions(e:any){
        ...
        removePermission(permission, params.row._id).then(result => {
          if(!result.isError){
 ...
 {params.row.permissions.length > 0 ? (
  ...
  <div className='flex gap-3 '>
    <select className=' border border-[#00B0AD] px-3 py-4 rounded-md bg-white text-[#00B0AD] ' onChange={removePermissions}>
  ...
  <select className=' border border-[#00B0AD] px-3 py-4 rounded-md bg-white text-[#00B0AD] ' onChange={addPermissions}>
