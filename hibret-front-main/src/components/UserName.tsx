              ...
              {notifications.length === 0 ? (
                ...
                notifications.map((notification, index) => (
                  <div key={index} className="flex gap-2 items-center p-2 border-b border-opacity-20">
                    ...
                    <div>
                      <p className="text-sm text-gray-400"> <a className="text-teal-400">{notification.workflowName}</a> was assigned to you</p>
                    </div>
                  </div>
                ))
              )}
              {notifications.length > 0 && <a href="/notification"><button className="border-npne h-8 text-teal-400 text-sm px-5">View all notification</button></a>}
            </div>
          )}
        </div>
          ...
          {isOpen ? (
            <div className="flex flex-col justify-center items-center gap-2 absolute top-14 right-2 h-auto w-44 py-4 border bg-white rounded-md shadow-lg">
              <a href="/changePassword" className="w-full flex justify-center">
                <button className="border text-xs border-gray-300 w-32 py-1 text-gray-500 hover:bg-gray-100 rounded-md">Change Password</button>
              </a>
              <button className="border w-32 py-1 text-xs text-white bg-red-500 hover:bg-red-600 rounded-md" onClick={handleLogout}>Log Out</button>
            </div>
          ) : <div> </div>}
        </div>
      </div>
